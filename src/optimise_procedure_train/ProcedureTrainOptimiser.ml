...
let corresponding_position(d:p_dict)(l_position:int):int=
  List.assoc l_position d
...
let process(a:FlowAction.t)(s:state):(state*FlowAction.t list*FlowAction.t list)=
  ...
  | (_,AfterLabeledPosition l_position,d)->((NormalState,d),[FlowAction.SkipProcedureLabeledPosition (corresponding_position dict l_position);a],[])
