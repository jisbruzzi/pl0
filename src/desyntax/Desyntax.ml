...
let generate_passes tl=
  let labels,token=tl in
  match List.rev labels with
  | hd::tl->ContextChange.Passes(hd,token)
  | [] -> ContextChange.Passes(SyntaxLabel.NoLabel,token)
...
let rec label_changes (new_labels:SyntaxLabel.t list)(old_labels:SyntaxLabel.t list):ContextChange.t list=
  ...
  | _ -> List.concat [ List.rev (generate_exits old_labels); generate_arrivals new_labels ]
...
let desyntax(tl:TokenWithLabels.t)(s:desyntax_state):(desyntax_state*ContextChange.t list*TokenWithLabels.t list)=
  ...
  (labels,List.concat [ context_changes; [generate_passes tl] ],[])
