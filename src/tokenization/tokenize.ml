...
let run (file:CharWithCoords.t Lazylist.gen_t):token_coords Lazylist.gen_t =
  LazylistOps.run process file Initial


let rec check (tokens:token_coords Lazylist.gen_t):token_coords Lazylist.gen_t=
  LazylistOps.stateless_run (fun t ->
    (match t with
      | (_,(Token.BadString(_)|Token.Nul(_)|BadInteger(_))) -> raise (BadTokenException.BadTokenException(t))
      |  _ -> ()
    );Some(t)
  ) tokens